...
CUSTOM_DOC("Responding to a startup event")
{
  ...
  if(match_core_code(&input, CoreCode_Startup)){
    ...
    byp_get_or_open_build_panel(app); // Disabled to use regular build panel
  ...
  Color_Table *table = get_color_table_by_name(string_u8_litexpr("theme-nasr"));
...
byp_render_caller(Application_Links *app, Frame_Info frame_info, View_ID view_id){
  ...
  byp_render_buffer(app, view_id, face_id, buffer, text_layout_id, region, frame_info);
