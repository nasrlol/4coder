...
load_config_and_apply(Application_Links *app, Arena *out_arena, i32 override_font_size, b32 override_hinting){
    ...
    if (file == 0){
        print_message(app, string_u8_litexpr("DEBUG: config.4coder file not found in search paths\n"));
    } else {
        print_message(app, string_u8_litexpr("DEBUG: config.4coder file found\n"));
    }
    ...
    if (file != 0){
        ...
        if (data.str != 0){
            ...
            print_message(app, string_u8_litexpr("DEBUG: failed to read config file data\n"));
        }
    }
